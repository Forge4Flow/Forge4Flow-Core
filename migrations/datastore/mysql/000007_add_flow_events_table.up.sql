...
CREATE TABLE IF NOT EXISTS flowEvent (
  ...
  ownerField varchar(64) NULL,
  script text NULL,
  removeAction bit NULL,
  actionEnabled bit NOT NULL DEFAULT 0,
  ...
  CONSTRAINT check_fields CHECK ((actionEnabled = 1) OR (objectType IS NOT NULL AND objectId IS NOT NULL AND ownerField IS NOT NULL AND script IS NOT NULL AND removeAction IS NOT NULL))
